package de.nikem.playground.qt;
...
import com.apple.laf.AquaProgressBarUI.Animator;
...
public class LiveCam {
	...
	/**
	 * Initializes the SequenceGrabber. Gets it's source video bounds, creates a gWorld with that size. Configures the video channel for grabbing, previewing and playing during recording.
	 */
	...
	/**
	 * This initializes the buffered image. First if determines the size of the data. Finds the number of ints per row, sets up the int array, where we can put the pixel data in. A DataBuffer is defined, using that int array. Based on this
	 * DataBuffer the BufferedImage is defined. The BufferedImage definition is not needed when the OpenGL view is used. There we only need the int-array pixelData. But I do not expect that defining the Buffered image has negative influence
	...
	/**
	 * This is a bit tricky. We do not start Previewing, but recording. By setting the output to a dummy file (which will never be created (hope so)) with the quicktime.std.StdQTConstants.seqGrabDontMakeMovie flag set. This seems to be
	 * equivalent to preview mode with the advantage, that it refreshes correctly.
	 */
	...
	/**
	 * This creates a Panel, which displays the buffered image using awt. A Thread is started, copying the pixel data from the sequence grabbers gWorld to the data buffer of the BufferedImage. Then the image is repainted.
	 */
